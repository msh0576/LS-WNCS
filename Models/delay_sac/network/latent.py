...
from MPN_project.delay_sac.utils import build_mlp, calculate_kl_divergence
...
class Decoder(torch.jit.ScriptModule):
    ...
    def __init__(self, input_dim=288, output_dim=3, std=1.0, use_image=False):
    ...
    def forward(self, x):
        ...
        if self.use_image:
            x = x.view(B * S, latent_dim, 1, 1)   # for image
            x = self.net(x)
            _, C, W, H = x.size()
            x = x.view(B, S, C, W, H)
        else:
...
class Encoder(torch.jit.ScriptModule):
    ...
    def __init__(self, input_dim=3, output_dim=256, use_image=True):
    ...
    def forward(self, x):
        if self.use_image:
            # print("x.size:", x.size())
            # B: batch, S: sequence? trajectory?, [C, H, W]: image size
            B, S, C, H, W = x.size()
            x = x.view(B * S, C, H, W)
        else:
            # B: batch, S: sequence? trajectory?, D: state size
            B, S, D = x.size()
            x = x.view(B * S, D)
        x = self.net(x)
        x = x.view(B, S, -1)
...
_AVAILABLE_ENCODERS = {'pixel': Encoder, 'identity': IdentityEncoder}

def make_encoder(
    encoder_type, input_dim, output_dim, use_image
):
    assert encoder_type in _AVAILABLE_ENCODERS
    return _AVAILABLE_ENCODERS[encoder_type](
        input_dim, output_dim, use_image
    )
...
class LatentModel(torch.jit.ScriptModule):
    ...
    def __init__(
        ...
        # feat(t) = Encoder(x(t))
        print("state_shape[0]:{} | feature_dim:{}".format(state_shape[0], feature_dim))

        self.encoder = Encoder(state_shape[0], feature_dim, use_image=use_image)
        # self.encoder = make_encoder(encoder_type, state_shape[0], feature_dim, use_image)
    ...
    def sample_prior(self, actions_, z2_post_):
        ...
        # z2_post_: [B, T+1, D_feature] | z1_mean_: [B, T, 32?] | actions_: [B, T, D_action]
        # print(f"z2_post_:{z2_post_.shape} | z1_mean_{z1_mean_.shape} | actions_:{actions_.shape}")
    ...
    def calculate_loss(self, state_, action_, reward_, done_):
        ...
        # state_: [B, T+1, D_state] | action_: [B, T, D_action] | reward_: [B, T, 1] | feature_: [B, T, D_feature]

        # Sample from latent variable model.
        z1_mean_post_, z1_std_post_, z1_, z2_ = self.sample_posterior(feature_, action_)    # z1: [B, T+1, 32(B?)] | z2: [B, T+1, D_feature]
        # print(f"z1_:{z1_.shape} | z2_:{z2_.shape}")
        
        z1_mean_pri_, z1_std_pri_ = self.sample_prior(action_, z2_) # [B, T+1, 32] | [B, T+1, 32]
        # print(f"z1_mean_pri_:{z1_mean_pri_.shape} | z1_std_pri_:{z1_std_pri_.shape}")
        ...
        # print(f"loss_image:{loss_image} | loss_reward:{loss_reward}")
