...
from Models.pdqn.agent import Agent
from Models.pdqn.memory.memory import Memory
from Models.pdqn.utils import soft_update_target_network, hard_update_target_network
from Models.pdqn.utils.noise import OrnsteinUhlenbeckActionNoise
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        print("============================")
        print(f"action_space:{action_space} | action_space.shape:{action_space.shape}")
        print(f"observation_space:{observation_space} | observation_shapce.shape:{observation_space.shape}")
        self.device = device
        # self.num_actions = self.action_space.spaces[0].n
        self.num_actions = schedule_size    # discrete action size
        # self.num_actions = sf_len    # discrete action size = maximum period = superframe length
        self.sf_len = sf_len

        self.action_parameter_sizes = np.array([sf_len for i in range(1,self.num_actions+1)])    # element is continuous action size / 
        # self.action_parameter_sizes = np.array([sf_len for i in range(1,self.num_actions+1)])    # element is continuous action size / 
        ...
        # self.action_parameter_max_numpy = np.concatenate([self.action_space.high for i in range(1,self.num_actions+1)]).ravel()
        # self.action_parameter_min_numpy = np.concatenate([self.action_space.low for i in range(1,self.num_actions+1)]).ravel()
        self.action_parameter_max_numpy = np.concatenate([self.action_space.high for i in range(1,(self.num_actions*sf_len)+1)]).ravel()
        self.action_parameter_min_numpy = np.concatenate([self.action_space.low for i in range(1,(self.num_actions*sf_len)+1)]).ravel()
        ...
        print(f"weighted:{weighted} | average:{average} | random_weighted:{random_weighted}")
        ...
        print(f"self.num_actions+self.action_parameter_size:{self.num_actions+self.action_parameter_size}")
        ...
        # network scheduler
        # self.random_scheduler = scheduler(algo='random', schedule_len=num_plant+1, max_period=schedule_size-1)
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            all_action_parameters = np.clip(all_action_parameters, -1.0, 1.0)   # clamp [-1., 1.]
    ...
    def random_act(self):
        '''
        Output:
            act: discrete action, scalar
            all_action_parameters: continuous actions, length = discrete action size
            act_param: all_action_parameters[act]: select system's action
        '''
        action = self.np_random.choice(self.num_actions)
        # action =2
        all_action_parameters = torch.from_numpy(np.random.uniform(self.action_parameter_min_numpy,
                                                    self.action_parameter_max_numpy)).type(torch.float)
        # add noise only to parameters of chosen action
        all_action_parameters = all_action_parameters.cpu().data.numpy()
        offset = np.array([self.action_parameter_sizes[i] for i in range(action)], dtype=int).sum()
        if self.use_ornstein_noise and self.noise is not None:
    ...
    def step_env(self, env, t, act, act_param, all_action_parameters, is_random=False, delay_step_=False):
        t += 1
        dt = act + 1    # preprocess the scheduling
        # print(f"act:{act} | act_param:{act_param} | all_action_parameters:{all_action_parameters}")
        

        if not delay_step_:

            next_state, reward, terminal, _ = dt_step(env, act_param, dt)
            next_state = np.array(next_state, dtype=np.float32, copy=False)
            # print(f"next_state:{next_state} | terminal:{terminal}")
            if not is_random:
                next_act, next_act_param, next_all_action_parameters = self.act(next_state)
            else:
                next_act, next_act_param, next_all_action_parameters = self.random_act()
                # print(f"next_act:{next_act}")
            # print(f"next_act_param:{next_act_param}")
            # next_action = self.pad_action(next_act, next_act_param)
            self.step(self.prev_state, (act, all_action_parameters), reward, next_state,
        ...
        return dt, reward, terminal, next_act, next_act_param, next_all_action_parameters

    def sf_step(self, env, t, is_random, act, act_param, all_action_parameters, delay_step_=False):
        '''
        this function is executed in units of superframe
        '''
        sf_reward = 0.
        cactions = []
        # == superframe schedule ==
        action, action_parameters, all_action_parameters = self.sf_exploration(self.prev_state)
        # print(f"shape:{action} | {action_parameters} | {all_action_parameters}")
        cactions, dts = self.postprocess(action_parameters, action)
        ...
        for idx, dt in enumerate(dts):
            ...
            if is_random:
                ...
                # print(f"caction:{caction} | dt:{dt}")

            # == step env ==
            state, reward, terminal, _ = dt_step(env, caction, dt=dt)
        ...
        next_act, next_act_param, next_all_action_parameters = self.act(state)
        self.step(self.prev_state, (act, all_action_parameters), reward, state,
                    (next_act, next_all_action_parameters), terminal)
        self.prev_state = state
        
        return t, sf_reward, terminal, next_act, next_act_param, next_all_action_parameters

    def evaluate_sf_steps(self, env, delay_step_=False):
        state = env.reset()
        prev_state = state
        episode_return = 0.0
        done = False
        dt_ratio = [0 for _ in range(self.num_actions+1)]
        caction_list = []


        while not done:
            # == superframe schedule ==
            action, action_parameters, all_action_parameters = self.sf_exploration(prev_state)
            cactions, dts = self.postprocess(action_parameters, action)
    ...
    def sf_exploration(self, obs):
        '''
        Input:
            obs: np, [D_state]
        Output:
            action_vec: np, [sf_len*D_action, ]
        '''
        if type(obs) != torch.tensor:
            obs = torch.tensor(obs, dtype=torch.float, device=self.device)

        with torch.no_grad():
            all_action_parameters = self.actor_param.forward(obs)
            # action_vec, _, period, _ = self.sf_policy.get_action(obs)
            # print(f"action_vec:{action_vec} | period:{period}")
            rnd = self.np_random.uniform()
            if rnd < self.epsilon:
                action = self.np_random.choice(self.num_actions)
                if not self.use_ornstein_noise:
                ...
                # select maximum action
                Q_a = self.actor.forward(obs.unsqueeze(0), all_action_parameters.unsqueeze(0))
                Q_a = Q_a.detach().cpu().data.numpy()
                action = np.argmax(Q_a)
    ...
    def postprocess(self, action_vec, period):
        '''
        Input:
            action_vec: np, [sf_len*D_action]
            period: np, [1,]
        Output:
            actions: np, [1, num_used_action]
            dts: list, 
        '''
        dt = period + 1
        # === number of action that will be used during a SF ===
        num_sf_action = self.sf_len // dt     
        if self.sf_len % dt > 0:
            num_sf_action += 1
        # === periods during a SF ===
        dts = [dt] * (num_sf_action - 1)    
        dt_resid = self.sf_len - sum(dts)
        if dt_resid > 0:
            dts.append(dt_resid)
        # print(f"dt:{dt} | num_sf_action:{num_sf_action} | sf_len:{self.sf_len} | dts:{dts}")

        return action_vec[:num_sf_action], dts

    def evaluate_steps(self, env, delay_step_=False):
        state = env.reset()
        state = np.array(state, dtype=np.float32, copy=False)
        act, act_param, all_action_parameters = self.act(state)
        done = False
        episode_return = 0.0
        dt_ratio = [0 for _ in range(self.num_actions)]
        cont_cunt = 0
        ...
        return episode_return, dt_ratio, cont_cunt



    def pad_action(self, act, act_param):
        params = [np.zeros((1,), dtype=np.float32) for _ in range(self.num_actions)]
    ...
    def set_action_parameter_passthrough_weights(self, initial_weights, initial_bias=None):
        ...
        print(f"initial_weights:{initial_weights.shape}")
        print(f"passthrough_layer:{passthrough_layer.weight.data.size()}")
    ...
    def step(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        ...
        if 'nstep' in self.algo_name:
            nstep_return = True
            sample_inputs = (state, np.concatenate(([act],all_action_parameters)).ravel(), 
                reward, next_state, terminal
            )
        else:
            sample_inputs = (state, np.concatenate(([act],all_action_parameters)).ravel(), 
                reward, next_state, np.concatenate(([next_action[0]],next_action[1])).ravel(), 
                terminal
            )
        # self._add_sample(state, np.concatenate((all_actions.data, all_action_parameters.data)).ravel(), reward, next_state, terminal)
        self._add_sample(*sample_inputs)
        # print(f"[1]:{state} | [2]:{np.concatenate(([act],all_action_parameters)).ravel()} | [3]:{reward} | [4]:{next_state} | [5]:{np.concatenate(([next_action[0]],next_action[1])).ravel()}" )
    ...
    def _optimize_td_loss(self):
        ...
        # print(f"delta_a:{delta_a.shape} | action_params:{action_params.shape}")
