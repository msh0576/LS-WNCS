...
from MPN_project.pdqn.memory.memory import MemoryNStepReturns
from MPN_project.pdqn.algo_pdqn import PDQNAgent
from MPN_project.pdqn.utils import soft_update_target_network
...
class PDQNNStepAgent(PDQNAgent):
    ...
    def _add_sample(self, state, action, reward, next_state, terminal, n_step_return=True):
        assert len(action) == 1 + self.action_parameter_size
        assert n_step_return is not None
        # print(f"state:{state} | action:{action} |reward:{reward} | next_state:{next_state} | terminal:{terminal}")
